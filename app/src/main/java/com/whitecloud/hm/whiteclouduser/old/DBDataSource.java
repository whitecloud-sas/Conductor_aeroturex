...
public class DBDataSource {
    int v_cantservicios = 200, v_cantmensajes = 200;
    // Database fields
...
//		log("Eliminando todos los registros de "+DBHelper.TABLA);
        ...
        while (!cursor.isAfterLast()) {
            try {
                ...
                if (tipo.equals("servicio")) {
                    ...
                    if (v_contador > v_cantmensajes) {
                        database.delete(DBHelper.TABLA, DBHelper.COLUMNA_ID + " = " + id, null);
                        log("Eliminando por exceso de mensajes:" + v_cantmensajes + "|contador: " + v_contador);
                    } else {
                        JSONObject list1 = new JSONObject(descripcion);
                        jArr.put(list1);
                        //actualiza el id de la lista
    ...
    public JSONArray getAll_mensajes(JSONObject jObj) {
        ...
        //a partir del id de postgres, traemos el origen del mensaje
        Cursor cursor1 = database.rawQuery("SELECT m_origen FROM mensajes WHERE m_id_postgres='" + m_id_postgres + "'", null);
        cursor1.moveToFirst();
        while (!cursor1.isAfterLast()) {
            m_origen = cursor1.getString(0);
            cursor1.moveToNext();
        }
        cursor1.close();*/

        //traemos todos los mensajes de ese origen
        String sql ="SELECT m_id,m_descripcion,m_me FROM mensajes WHERE m_origen='" + m_origen + "' ORDER BY m_id ASC";
        log(sql);
    ...
    public int selectUbicacionPendientes() {
        ...
        String sql = "SELECT su_fecha,su_servicio,su_id,su_distancia,su_precio," +
                ...
                "FROM da " +
                "WHERE " + DBHelper.COLUMNA_TIPO + "='ruta' " +
                "and su_id='0' " +
    ...
    public void createUbicacion(JSONObject jObj) {
        try {
            ...
            values.put(DBHelper.COLUMNA_TIPO, "ruta");
            values.put(DBHelper.COLUMNA_DESCRIPCION, "ruta");
